...
async function sendPush(token, title, body, tag, vibrate, tokenField) {
  if (!token) return;
  const isWiebke = tokenField === 'wiebkeFcmToken';
  const msg = {
    token,
    data: {
      title: title || 'Timer7',
  ...
  // notification field: system displays even if native app is force-stopped
  // Only for native Android (Moritz). Web PWA (Wiebke) has SW push handler → would duplicate.
  if (!isWiebke) {
    msg.notification = { title: title || 'Timer7', body: body || '' };
    msg.android = { priority: 'high', ttl: 120000,
      notification: { channelId: tag && (tag.includes('alarm') || tag.includes('rabbit') || tag.includes('nudge') || tag.includes('approval')) ? 'timer7_timer_alarm' : 'timer7_fcm', sound: 'default' }
    };
  } else {
    msg.android = { priority: 'high', ttl: 120000 };
  }
  try {
    ...
    if (err.code === 'messaging/registration-token-not-registered' ||
      ...
      console.error('sendPush failed:', err.code || err.message);
    }
  }
}
...
exports.onNudge = functions
  ...
  .onWrite(async (change) => {
    ...
    // Don't nudge during work
    const statusSnap = await admin.database().ref(STATUS_PATH).once('value');
    const s = statusSnap.val();
    if (s && (s.isAtWork === true || s.status === 'working')) return;
...
// ════════════════════════════════════════════
// 3. Wiebke Freiminuten (idleApproval) — FCM push to wake native app
// ════════════════════════════════════════════
exports.onIdleApproval = functions
  .region(REGION)
  .database.instance('pomodoro-status-default-rtdb')
  .ref(STATUS_PATH + '/idleApproval')
  .onWrite(async (change) => {
    const data = change.after.val();
    if (!data || !data.approved || !data.time) return;
    if (Date.now() - data.time > 60000) return;
    const tokenSnap = await admin.database().ref(STATUS_PATH + '/fcmToken').once('value');
    const mins = data.durationMin || 30;
    const reason = data.reason || '';
    await sendPush(tokenSnap.val(),
      '\u{1F91D} Wiebke: ' + mins + ' min Pause',
      reason ? reason + ' \u2014 ' + mins + ' min genehmigt' : mins + ' min Freiminuten genehmigt',
      'wiebke-approval', NORMAL_VIBRATE);
  });
...
async function pushMoritz(d, token, status, staleMin, appDead, appVeryDead, awake, pastWake) {
    ...
    if (status === 'break') {
      if (d.breakEnd && Date.now() > d.breakEnd) {
        ...
        await sendPush(token,
          overMin > 0 ? '\u{1F480} Anki-Spaziergang vorbei! +' + overMin + 'm idle' : '\u{1F6B6} Anki-Spaziergang vorbei!',
        ...
        await sendPush(token,
          '\u26A0\uFE0F Anki-Spaziergang-Timer evtl. gestoppt!',
          'App seit ' + Math.round(staleMin) + 'min offline. Noch ' + remMin + 'min Anki-Spaziergang.',
          rotTag('idle-nag'), NORMAL_VIBRATE);
      }
      return;
    }
